...
Platform_L_vertical::Platform_L_vertical(float posX, float posY, float width, float height)
    : Platform_L(posX, posY, width, height)
{
        setVelocityY(.2f);
...
void Platform_L_vertical::movePlatform()
{
        ...
        if (abs(getY() - getStartPositionY()) >= 0.25f)
                changeDirection();
        Notify();
}

void Platform_L_vertical::changeDirection() { setVelocityY(-getVelocityY()); }

Platform_L_horizontal::Platform_L_horizontal(float posX, float posY, float width, float height)
    : Platform_L(posX, posY, width, height)
{
        setVelocityX(.2f);
...
void Platform_L_horizontal::movePlatform()
{
        ...
        if (getX() <= 0)
                changeDirection();
        else if (getX() + getWidth() >= 1)
                changeDirection();
        Notify();
}

void Platform_L_horizontal::changeDirection() { setVelocityX(-getVelocityX()); }

Platform_L_temporary::Platform_L_temporary(float posX, float posY, float width, float height)
    : Platform_L(posX, posY, width, height)
{
        setTemporary(true);
}
