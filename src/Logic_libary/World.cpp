...
World::World()
{
        ...
        m_playerHeight = .125f;
        m_playerWidth = .1f;
        m_platformHeight = 0.035f;
        m_platformWidth = .15f;
        m_springHeight = .05f;
        m_springWidth = .05f;
        m_rocketHeight = .075f;
        m_rocketWidth = .075f;
        m_bgTileHeight = abs(m_belowLogicY) * 3;
        m_bgTileWidth = abs(m_leftLogicX) + abs(m_rightLogicX);
...
void World::setFactory(shared_ptr<EntityFactory>& factory) { m_entityFactory = move(factory); }
...
bool World::getGameStatus() { return m_isGamePlaying; }
...
bool World::Collision(vector<pair<float, float>>& movedSubjectpath, const shared_ptr<A>& movedSubject,
                      const shared_ptr<B>& subject, bool goesUp)
{
        ControllingPointers::control(subject, "World",
                                     "Collision(vector<pair<float, float>> &leftPlayer, vector<pair<float, float>> "
                                     "&rightPlayer, const shared_ptr<T>& subject)");
        ...
        for (int i = 0; i < movedSubjectpath.size(); i++) {
                ...
                if (goesUp)
                        movedSubject_y += movedSubject->getHeight();
                ...
                if (subject_Y0 <= movedSubject_y && movedSubject_y <= subject_Y1) {
                        ...
                        if ((biggest_X0 <= smallest_X0 && smallest_X0 <= biggest_X1) ||
                            (biggest_X0 <= smallest_X1 && smallest_X1 <= biggest_X1)) {
                                return true;
                        }
                }
        }
...
void World::getPointsBetweenFrames(vector<pair<float, float>>& left, const shared_ptr<T>& subject)
{
        ...
        ControllingPointers::control(subject, "World",
                                     "getPointsBetweenFrames(vector<pair<float, float>> &left, vector<pair<float, "
                                     "float>> &right, const shared_ptr<Player_L>& subject)");
...
{
        // controlen voor type
        if (is_same<T, Player_L>::value || is_same<T, Platform_L>::value || is_same<T, BG_Tile_L>::value ||
            is_same<T, Bonus_L>::value) {
...
void World::createPlayer(shared_ptr<EntityFactory>& factory)
{
        ...
        m_player = move(std::make_shared<logic::Player_L>(
            (m_leftLogicX + m_rightLogicX) / 2, m_belowLogicY + m_playerHeight, m_playerWidth, m_playerHeight));
...
{
        ControllingPointers::control(
            m_player, "World",
            "playerTouchesPlatform(vector<pair<float, float>>& leftPlayer, vector<pair<float, float>>& rightPlayer)");
        ...
        for (auto& platform : m_platforms) {
                ...
                ControllingPointers::control(platform, "World",
                                             "playerTouchesPlatform(vector<pair<float, float>>& leftPlayer, "
                                             "vector<pair<float, float>>& rightPlayer)");
...
void World::playerTouchesBoost(vector<pair<float, float>>& playerPath)
{
        ControllingPointers::control(
            m_player, "World",
            "playerTouchesBoost(vector<pair<float, float>>& leftPlayer, vector<pair<float, float>>& rightPlayer)");

        for (auto& bonus : m_bonussen) {
                ControllingPointers::control(bonus, "World",
                                             "playerTouchesBoost(vector<pair<float, float>>& leftPlayer, "
                                             "vector<pair<float, float>>& rightPlayer)");

                if (Collision(playerPath, m_player, bonus)) {
                        if (bonus->getInvolmsVelocity() && m_player->getVelocityY() <= 0 &&
                            !m_player->getBonusActive()) {
...
{
        ControllingPointers::control(factory, "World",
                                     "createPlatform(shared_ptr<EntityFactory> &factory, float x, float y)");
...
void World::createAplatform(shared_ptr<EntityFactory>& factory, bool begin)
{
        ...
        ControllingPointers::control(factory, "World",
                                     "createAplatform(shared_ptr<EntityFactory> &factory, bool begin)");
        ControllingPointers::control(m_player, "World",
                                     "createAplatform(shared_ptr<EntityFactory> &factory, bool begin)");
...
int World::getHighScore() { return m_highScore; }
...
int World::getScore() { return m_prevScore; }
...
void World::saveScore()
{
        ...
        if (m_score->getRecalculatedScore() > highscore)
                highScoreJson["highscore"] = m_score->getRecalculatedScore();
        else
...
{
        ControllingPointers::control(factory, "World",
                                     "createBG_Tile(shared_ptr<EntityFactory> &factory, bool second)");

        shared_ptr<logic::BG_Tile_L> tile;
        if (second) {
                tile = move(std::make_shared<logic::BG_Tile_L>(
                    m_leftLogicX, (abs(m_belowLogicY) * 2) - abs(m_belowLogicY), m_bgTileWidth, m_bgTileHeight));
        } else {
                tile = move(
                    std::make_shared<logic::BG_Tile_L>(m_leftLogicX, m_belowLogicY, m_bgTileWidth, m_bgTileHeight));
        }
...
void World::refreshBg_Tile() { tileOutOfView(); }
...
void World::createBonus(shared_ptr<EntityFactory>& factory, float x, float y)
{
        ...
        case Rocket_:
                ...
                bonus =
                    move(std::make_shared<logic::Rocket_L>(x1, y + m_platformHeight, m_rocketWidth, m_rocketHeight));
        }
